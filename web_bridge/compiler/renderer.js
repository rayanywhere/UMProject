...
function renderAsPHPServer(root) {
	...
	root.versions.forEach(function(version) {
				var keyName = version.name + "/UMWebBridgeServer.php";
				var file = {
				};
				file[keyName] = swig.renderFile(__dirname + "/templates/php/server/UMWebBridgeServer.php", {version:version});
				files.push(file);
			});
	return files;
}
...
function renderAsPHPClient(root) {
	...
	return [
			{
				"UMWebBridgeClient.php": swig.renderFile(__dirname + "/templates/php/client/UMWebBridgeClient.php", root)
			}
		];
}
...
function renderAsObjcClient(root) {
	...
	return [
				{
					"UMWebBridgeClient.h": swig.renderFile(__dirname + "/templates/objc/client/UMWebBridgeClient.h", root),
					"UMWebBridgeClient.m": swig.renderFile(__dirname + "/templates/objc/client/UMWebBridgeClient.m", root)
				}
			];
}
